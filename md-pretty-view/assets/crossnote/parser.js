({
  ...
  onWillParseMarkdown: async function(markdown) {
    // Auto-inject the Mermaid pan/zoom script into every document as a
    // whitelisted `@import` (the only way MPE will run custom JS — scripts
    // in head.html are stripped). Path starting with "/" resolves from the
    // project root. Requires enableScriptExecution: true.
    if (markdown.indexOf("mermaid-panzoom.js") !== -1) return markdown;
    var imp = '@import "/.crossnote/mermaid-panzoom.js"\n\n';

    // Front-matter must stay at the very top: insert the import AFTER it.
    if (markdown.slice(0, 3) === "---") {
      var end = markdown.indexOf("\n---", 3);
      if (end !== -1) {
        var nl = markdown.indexOf("\n", end + 1);
        var pos = nl === -1 ? markdown.length : nl + 1;
        return markdown.slice(0, pos) + "\n" + imp + markdown.slice(pos);
      }
    }
    return imp + markdown;
  },
  ...
  onDidParseMarkdown: async function(html) {
    // The `@import` above only runs in the LIVE PREVIEW: MPE lists injected
    // JS in `JSAndCssFiles`, which the *preview* template emits as <script>
    // tags — but the HTML-EXPORT / "Open in Browser" template drops that list
    // entirely. So for export we inline the very same script into the body.
    //
    // Why inline (not <script src>)? This parser runs in a sandboxed JS
    // runtime with no `fs`/`require`/`__dirname`, and it has no way to learn
    // the project's absolute path — so it can neither read the .js file nor
    // build a portable src. Inlining keeps exported HTML self-contained.
    //
    // Preview is unaffected: the exported markup is applied there via
    // innerHTML (see preview.js `H.current.innerHTML = data-html`), and
    // browsers never execute <script> inserted through innerHTML.
    //
    // ⚠️  KEEP PZ_SOURCE BELOW BYTE-FOR-BYTE IN SYNC WITH
    //     .crossnote/mermaid-panzoom.js (that file is the canonical copy the
    //     preview @import loads; this string is only used for export).
    if (html.indexOf("__mmdPzDelegated") !== -1) return html; // already injected
